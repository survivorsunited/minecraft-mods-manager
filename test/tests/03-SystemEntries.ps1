...
Test-Command "& '$ModManagerPath' -AddMod -AddModName 'Minecraft Server' -AddModType 'server' -AddModGameVersion '1.21.5' -AddModUrl 'https://piston-data.mojang.com/v1/objects/e6ec2f64e6080b9b5d9b471b291c33cc7f509733/server.jar' -AddModVersion '1.21.5' -AddModJar 'minecraft_server.1.21.5.jar' -DatabaseFile '$TestDbPath' -ApiResponseFolder '$script:TestApiResponseDir'" "Add Minecraft Server 1.21.5" 5 $null $TestFileName
Test-Command "& '$ModManagerPath' -AddMod -AddModName 'Minecraft Server' -AddModType 'server' -AddModGameVersion '1.21.6' -AddModUrl 'https://piston-data.mojang.com/v1/objects/6e64dcabba3c01a7271b4fa6bd898483b794c59b/server.jar' -AddModVersion '1.21.6' -AddModJar 'minecraft_server.1.21.6.jar' -DatabaseFile '$TestDbPath' -ApiResponseFolder '$script:TestApiResponseDir'" "Add Minecraft Server 1.21.6" 6 $null $TestFileName

# Test 2: Test system entry filename handling (verify Jar column is used)
Write-TestHeader "Test System Entry Filename Handling"
Test-Command "& '$ModManagerPath' -AddMod -AddModName 'Test Server' -AddModType 'server' -AddModGameVersion '1.21.6' -AddModUrl 'https://example.com/server.jar' -AddModVersion '1.21.6' -AddModJar 'test_server_1.21.6.jar' -DatabaseFile '$TestDbPath' -ApiResponseFolder '$script:TestApiResponseDir'" "Add Test Server with Jar filename" 7 $null $TestFileName

# Test 3: Validate with UseLatestVersion (should detect majority version)
Write-TestHeader "Validate with UseLatestVersion"
Test-Command "& '$ModManagerPath' -ValidateAllModVersions -DatabaseFile '$TestDbPath' -UseLatestVersion -UseCachedResponses -ApiResponseFolder '$script:TestApiResponseDir'" "Validate with UseLatestVersion" 7 $null $TestFileName
...
Test-Command "& '$ModManagerPath' -DownloadMods -DatabaseFile '$TestDbPath' -UseLatestVersion -UseCachedResponses -ApiResponseFolder '$script:TestApiResponseDir'" "Download with UseLatestVersion" 7 $null $TestFileName

# Test 5: Download with UseLatestVersion and validation
Write-TestHeader "Download with UseLatestVersion and Validation"
Test-Command "& '$ModManagerPath' -DownloadMods -DatabaseFile '$TestDbPath' -UseLatestVersion -ValidateWithDownload -UseCachedResponses -ApiResponseFolder '$script:TestApiResponseDir'" "Download with UseLatestVersion and Validation" 7 $null $TestFileName
